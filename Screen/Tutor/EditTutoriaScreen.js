...
const EditTutoriaScreen = ({ navigation, route }) => {
  const [loading, setLoading] = useState(false);
  const [tutoria, setTutoria] = useState(route.params);
  ...
  const updateTutoria = async (id) => {
    ...
    fetch(`${environment.URL}/api/tutorias/${id}`, {
      ...
      .then(async (responseJson) => {
        ...
        navigation.navigate('TutoriasScreen', { tutoria })
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <View style={{ flex: 1, padding: 16 }}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.5}
          onPress={() => updateTutoria(tutoria.id)}>
...
export default EditTutoriaScreen;
